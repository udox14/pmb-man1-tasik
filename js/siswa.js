...
function renderProfile(data) {
  ...
  if (badgeJalur) {
    ...
    if (data.jalur === 'PRESTASI') {
      ...
      if (tabPrestasiBtn) tabPrestasiBtn.style.display = data.scan_sertifikat_prestasi_url ? 'inline-block' : 'none';
    }
  }
...
function renderStatus(data) {
  ...
  // D1 simpan boolean sebagai integer (1/0/null) — sudah dikonversi di API
  ...
  if (data.jalur === 'REGULER') {
    if (data.status_verifikasi === true) {
      if (data.ruang_tes && data.tanggal_tes && data.sesi_tes) {
        ...
        infoBox.style = "padding:20px; border-radius:12px; margin-bottom:20px; text-align:left;";
        ...
        if (isBebasTes) {
          ...
          infoBox.innerHTML = `
            <h4 style="margin:0 0 8px 0; display:flex; align-items:center; gap:8px;"><i class="ph ph-info" style="font-size:1.4rem;"></i> Informasi Ujian</h4>
            <p style="margin:0; font-size:0.95rem; line-height:1.5;">Status Anda: <strong>Jalur Reguler</strong>. Anda dinyatakan <strong>BEBAS TES TULIS (CBT)</strong> sebagai bentuk pengalihan dari Jalur Prestasi.</p>
          `;
        } else {
          ...
          infoBox.innerHTML = `
            <h4 style="margin:0 0 8px 0; display:flex; align-items:center; gap:8px;"><i class="ph ph-calendar-blank" style="font-size:1.4rem;"></i> Jadwal CBT Belum Tersedia</h4>
            <p style="margin:0; font-size:0.95rem; line-height:1.5;">Berkas Anda sudah <strong>VALID</strong>, namun jadwal ujian belum diatur. Tombol Cetak Kartu akan muncul setelah jadwal tersedia.</p>
          `;
        }
      ...
      if (data.status_verifikasi === null) {
        ...
        infoBox.style = "background:#fffbeb; border:1px solid #fcd34d; color:#b45309; padding:20px; border-radius:12px; margin-bottom:20px;";
        infoBox.innerHTML = `
          <h4 style="margin:0 0 8px 0; display:flex; align-items:center; gap:8px;"><i class="ph ph-warning-circle" style="font-size:1.4rem;"></i> Informasi Ujian</h4>
          <p style="margin:0; font-size:0.95rem;">Tombol <strong>Cetak Kartu Ujian</strong> belum tersedia. Tunggu hingga berkas diverifikasi panitia.</p>
        `;
        if (timerBanner) timerBanner.parentNode.insertBefore(infoBox, timerBanner.nextSibling);
      }
    }
...
window.prosesPengalihanReguler = async function (id) {
  ...
  if (confirm.isConfirmed) {
    ...
    try {
      ...
      Swal.fire('Berhasil', 'Status Anda telah dialihkan ke Jalur Reguler.', 'success').then(() => location.reload());
    } catch (e) {
  ...
  Swal.fire({
    ...
    html: `
      <div style="text-align:left; margin-top:10px;">
        ...
        <a href="https://wa.me/6281220338298" target="_blank" style="display:flex; align-items:center; gap:10px; padding:12px; text-decoration:none; color:#1e293b;">
...
function parseExamDate(tglStr, sesiStr) {
  try {
    ...
    let day = parts[0].padStart(2, '0'), month = parts[1], year = parts[2];
...
async function setupSmartCountdown(pendaftar) {
  ...
  if (pendaftar.jalur === 'PRESTASI') {
    ...
    if (now < tesPresDate) {
      targetDate = tesPresDate; titleText = "Menuju Tes Pembuktian Prestasi"; subText = "Harap hadir di kampus MAN 1 Tasikmalaya";
    } else {
      targetDate = new Date(tglPres).getTime(); titleText = "Menuju Pengumuman Hasil Prestasi"; subText = "Hasil Seleksi Jalur Prestasi";
    }
  } else if (pendaftar.jalur === 'REGULER') {
    if (isBebasTes) {
      targetDate = new Date(tglReg).getTime(); titleText = "Menuju Pengumuman Kelulusan Akhir"; subText = "Seleksi Menggunakan Nilai Rapor (Bebas Tes)";
    } else {
      if (pendaftar.tanggal_tes && pendaftar.sesi_tes) {
        ...
        if (examTime > now) {
          targetDate = examTime; titleText = "Menuju Ujian CBT Anda"; subText = `${pendaftar.tanggal_tes} - ${pendaftar.sesi_tes.split('(')[0].trim()}`;
        } else {
          targetDate = new Date(tglReg).getTime(); titleText = "Menuju Pengumuman Kelulusan Akhir"; subText = "Hasil Seleksi Jalur Reguler";
        }
      } else {
        targetDate = new Date(tglReg).getTime(); titleText = "Menuju Pengumuman Kelulusan Akhir"; subText = "Jadwal ujian belum tersedia, pantau terus";
      }
    }
  }
